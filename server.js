...
/* ---------- Dynamic sitemap.xml (live engine) ---------- */
const SITEMAP_STATIC = [
  { loc: "/",                          priority: "1.0",  changefreq: "monthly" },
  { loc: "/solutions/",                priority: "0.9",  changefreq: "monthly" },
  { loc: "/solutions/web-development/",priority: "0.9",  changefreq: "monthly" },
  { loc: "/solutions/ai-automation/",  priority: "0.9",  changefreq: "monthly" },
  { loc: "/solutions/ai-lead-generation/", priority: "0.9", changefreq: "monthly" },
  { loc: "/solutions/ai-calling-agent/",   priority: "0.9", changefreq: "monthly" },
  { loc: "/solutions/social-media-management/", priority: "0.8", changefreq: "monthly" },
  { loc: "/solutions/complete-growth-system/",  priority: "0.9", changefreq: "monthly" },
  { loc: "/work/",                     priority: "0.7",  changefreq: "monthly" },
  { loc: "/work/ironcore-fitness/",    priority: "0.6",  changefreq: "yearly"  },
  { loc: "/process/",                  priority: "0.6",  changefreq: "yearly"  },
  { loc: "/about/",                    priority: "0.6",  changefreq: "yearly"  },
  { loc: "/blog/",                      priority: "0.7",  changefreq: "weekly"  },
  { loc: "/contact/",                  priority: "0.9",  changefreq: "yearly"  },
  { loc: "/faq/",                      priority: "0.5",  changefreq: "yearly"  }
];

function xmlEscape(s) { return String(s).replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c])); }

async function serveDynamicSitemap(res) {
  try {
    const lines = [
      '<?xml version="1.0" encoding="UTF-8"?>',
      "<!-- Generated by SYNELIGHT — dynamic sitemap -->",
      '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'
    ];
    const push = (loc, chg, pri) =>
      lines.push("  <url><loc>" + SITE_URL + loc + "</loc><changefreq>" + chg + "</changefreq><priority>" + pri + "</priority></url>");
    SITEMAP_STATIC.forEach((p) => push(p.loc, p.changefreq, p.priority));
    try {
      const posts = await blog.list({ status: "published" });
      posts.forEach((p) => push("/blog/" + encodeURIComponent(p.slug) + "/", "monthly", "0.6"));
    } catch {}
    lines.push("</urlset>");
    serve(res, 200, "application/xml; charset=utf-8", lines.join("\n"));
  } catch { serverError(res, new Error("sitemap_generation_failed")); }
}
...
if (pathname.startsWith("/api/")) return json(res, 404, { success: false });
    ...
    /* ---- Dynamic sitemap (live blog engine) ---- */
    if (pathname === "/sitemap.xml" && req.method === "GET") {
      return serveDynamicSitemap(res);
    }
