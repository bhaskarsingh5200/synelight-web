...
async function relatedPostsHtml(currentId, currentCategory) {
  try {
    const all = await blog.list({ status: "published" });
    ...
    const cards = picks.map(function (p) {
      const cover = /^https?:\/\//.test(String(p.cover || ""))
        ? '<img class="insight-cover" src="' + blogrender.esc(p.cover) + '" alt="' + blogrender.esc(p.title) + '" loading="lazy">'
        : "";
      return '<article class="card insight-card">' +
        cover +
        '<p class="insight-cat">' + blogrender.esc(p.category) + "</p>" +
        '<h3 style="margin:0 0 10px;"><a href="/blog/' + encodeURIComponent(p.slug) + '/" style="color:inherit;text-decoration:none;">' + blogrender.esc(p.title) + "</a></h3>" +
        "<p>" + blogrender.esc(p.excerpt) + "</p>" +
        '<p class="insight-date">' + blogDate(p.date) + " · " + p.reading_minutes + " min read</p>" +
        '<a class="link-arrow" href="/blog/' + encodeURIComponent(p.slug) + '/" style="margin-top:18px;">Read Article <span class="arr" aria-hidden="true">→</span></a>' +
        "</article>";
    }).join("\n");
    return '<section class="section section--line" aria-labelledby="related-title">' +
      '<div class="container">' +
      '<div class="section-head section-head--center reveal">' +
      '<p class="eyebrow eyebrow--center">Keep Reading</p>' +
      '<h2 id="related-title" style="margin:0;">More from the blog</h2>' +
      '</div>' +
      '<div class="row-3-cols" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:28px;margin-top:48px;">' +
      cards +
      "</div>" +
      '<div class="btn-row reveal" style="margin-top:44px;justify-content:center;">' +
      '<a class="btn btn--ghost" href="/blog/">Browse All Articles <span class="arr" aria-hidden="true">→</span></a>' +
      "</div>" +
      "</div></section>";
  } catch { return ""; }
}
...
async function serveBlogPost(res, slug) {
  ...
  const related = await relatedPostsHtml(post.id, post.category);
  for (const key of Object.keys(tokens)) html = html.split(key).join(tokens[key]);
  if (related) html = html.split("__SL_RELATED__").join(related);
  return serve(res, 200, "text/html; charset=utf-8", html);
}
