...
/* Columns that exist on public.leads (keep in sync with supabase/migration.sql).
   Anything rowFrom() may add beyond these (e.g. referer) is dropped here —
   PostgREST rejects unknown columns with a 400. */
const LEAD_COLUMNS = [
  "id", "created_at", "updated_at", "full_name", "business_name", "email",
  "whatsapp", "website", "business_type", "service", "timeline", "budget",
  "description", "source", "utm_source", "utm_medium", "utm_campaign",
  "assigned_to", "status", "notes"
];

async function insert(row) {
  const clean = {};
  LEAD_COLUMNS.forEach((k) => { if (row[k] !== undefined) clean[k] = row[k]; });
  const rows = await rest("POST", "leads", "", clean, "return=representation");
  return rows && rows[0] ? rows[0] : null;
}
...
async function blogRows(filter) {
  const parts = ["select=*", "order=date.desc.nullslast"];
  if (filter && filter.status) parts.push("status=eq." + encodeURIComponent(filter.status));
