...
function ensure() {
  ...
  } catch { /* read-only fs (serverless) — in-memory only */ }
}
...
function writeAll(posts) {
  ...
  try {
    ...
    /* Read-only filesystem (serverless / Vercel): fall back to in-memory only.
       Production should use Supabase anyway — JSON is only a local fallback. */
    if (/EROFS|ENOSPC|ENOTDIR|EACCES|EPERM|EBUSY/.test(String(err.code))) return false;
    throw err;
  }
  return true;
}
...
function rowFrom(fields, existing) {
  ...
  return {
    ...
    cover: String(fields.cover || "").trim().slice(0, 500),
...
/* Shared patch logic used by both the JSON engine and the Supabase engine. */
function applyPostPatch(prev, patch, existing) {
  if (patch.title !== undefined) {
    const t = String(patch.title).trim();
    if (!t) return { error: "Title is required." };
    prev.title = t;
  }
  if (patch.slug !== undefined) prev.slug = uniqueSlug(slugify(patch.slug) || slugify(prev.title), existing, prev.id);
  ...
  if (patch.cover !== undefined) prev.cover = String(patch.cover || "").trim().slice(0, 500);
...
function update(id, patch) {
  ...
  const result = applyPostPatch(posts[idx], patch, existing);
...
module.exports = { list, getById, getBySlug, create, update, remove, slugify, rowFrom, applyPostPatch, uniqueSlug };